...
{"scores":{"intro":4,"conflict":3,"dialogue":4,"direction":2},"feedback":"잘한 점: ...\n보완할 점: ...\n교수 예상 반응: ...\n개선 한 문장: ..."}`;
  ...
  // ── 오늘의 참고 작품 ──
  if (mode === 'dailyRef') {
    const SCHEMA_VER = 2; // 구조 바꾸면 이 숫자만 올리면 됨
    const today = new Date().toISOString().slice(0, 10);
    const cacheKey = `dailyRef:${today}`;
    try {
      const cached = await kv.get(cacheKey);
      if (cached && cached._v === SCHEMA_VER) return res.status(200).json(cached);
      if (cached && cached._v !== SCHEMA_VER) await kv.delete(cacheKey).catch(()=>{}); // 구버전 삭제
    } catch(e) {}
    ...
    // 날짜 기반
    const dayIndex = new Date().getDate() % WORKS.length;
    const work = WORKS[dayIndex];
...
===JSON===
{"title":"${work.title}","ep":"${work.ep}","scene":"${work.scene}","genre":"${work.genre}","structurePattern":"구조패턴 설명","techniques":["기법1","기법2","기법3"],"dialogueStrategy":"대사전략 설명","sceneNum":"S#1","setting":"장소-시간","characters":["인물1","인물2"]}
===SCRIPT===
(여기에 오리지널 대본을 자유롭게 작성)`;
    ...
    try {
      ...
      if (jsonMatch && scriptMatch) {
        ...
        parsed = {
          ref: {
            title: meta.title, ep: meta.ep, scene: meta.scene, genre: meta.genre,
            structurePattern: meta.structurePattern, techniques: meta.techniques,
            dialogueStrategy: meta.dialogueStrategy
          },
          script: {
            sceneNum: meta.sceneNum, setting: meta.setting,
            characters: meta.characters, script: scriptText
          }
        };
      } else {
        // fallback
