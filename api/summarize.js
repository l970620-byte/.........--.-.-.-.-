...
{"terms":["검색어1","검색어2","검색어3","검색어4","검색어5","검색어6","검색어7","검색어8","검색어9","검색어10"]}`;
  ...
  if (mode === 'filter') {
    ...
    const prompt = `당신은 서울예대 방송영상전공 입시 전문 큐레이터입니다.
아래 기사 목록을 검토해 입시생에게 실질적으로 유용한 기사만 선별하세요.

${list}

[relevant:true — 80% 이상 부합해야 통과]
핵심 기준: 방송·미디어·OTT·콘텐츠·플랫폼·저널리즘·크리에이터 산업이 기사의 주제 그 자체여야 함
- 해당 산업의 정책·규제·시장 변화·기술 도입 기사
- 방송영상전공 입시 작문 주제로 직접 활용 가능한 기사
- 제목과 본문 모두 미디어 산업이 중심인 기사

[relevant:false — 즉시 탈락]
- 연예인 사생활·열애·화보·팬덤 (산업 이슈 아닌 것)
- 스포츠 경기결과·선수 이적
- 주식·코인·부동산·경제 일반
- 해외 정치·외교 (미디어 정책 아닌 것)
- 광고·홍보성 기사
- 제목에만 키워드 포함, 본문은 무관한 기사
- 미디어가 배경일 뿐 주제가 아닌 기사 (예: 드라마 OST 가수 인터뷰)

엄격하게 판단. 애매하면 false.
응답 형식 (JSON만, 다른 텍스트 없이):
{"results":[{"idx":0,"relevant":true,"summary":"입시생 시각 핵심 1줄 요약"},...]}`;
...
{"essay":{"p1":"intro paragraph","p2":"body1 paragraph","p3":"body2 paragraph","p4":"conclusion paragraph","tip":"one line strategy"}}`;
  ...
  // ── 뉴스 예상 문제 생성 ──
  if (mode === 'newsQuestion') {
    const { title, summary, keyword } = req.body;
    if (!title) return res.status(400).json({ error: 'title 필요' });
    const hashStr = crypto.createHash('sha256').update('nq|' + title).digest('hex').slice(0,24);
    const cacheKey = `nq:${hashStr}`;
    try { const cached = await kv.get(cacheKey); if (cached) return res.status(200).json({...cached, cached:true}); } catch(e) {}
    const prompt = `당신은 서울예대 방송영상전공 입시 작문 출제 교수입니다.
아래 뉴스 기사를 읽고 실제 입시 작문 시험에 나올 수 있는 문제를 1개 생성하세요.
키워드: "${keyword||''}"
제목: ${title}
요약: ${summary||''}

[출제 유형 — 매번 다른 유형 1개 선택]
기술설명형 / 현상분석형 / 기획안작성형 / 찬반논술형 / 해결책제안형 / 견해에세이형

JSON만 출력 (questions 배열에 1개만):
{"questions":[{"type":"유형명","question":"~서술하시오 형식의 문제 지문 (2~3줄)","hint":["포인트1","포인트2","포인트3"],"difficulty":"상·중·하 중 하나"}]}`;
    try {
      const response = await fetch('https://api.anthropic.com/v1/messages', {
        method:'POST',
        headers:{'Content-Type':'application/json','x-api-key':apiKey,'anthropic-version':'2023-06-01'},
      ...
      return res.status(500).json({ error: 'newsQuestion 실패: ' + e.message });
    }
  }
